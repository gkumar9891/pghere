...
if($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if(empty(trim($_POST["username"]))) {
     ...
     $sql = "select username from registerlandlord where username = ?";
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err) && empty($first_name_err) && empty($last_name_err) && empty($phone_no_err) && empty($email_id_err)) {
        ...
        echo "hello";
        $sql = "insert into register_tenant (first_name, last_name, phone_no, username, password, email_id) values (?, ?, ?, ?, ?, ?)";
            echo $sql;
        if($stmt = $mysqli->prepare($sql)) {
...
<head>
    ...
    <title>Signup Tenant | PG Here</title>
    <link rel="stylesheet" href="style.css">
</head>
<body>

    <div class="form">
        <h1 class="text-center">Signup as Tenant</h1>
